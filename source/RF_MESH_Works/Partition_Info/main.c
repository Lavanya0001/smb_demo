...
pico_partition_table_t pt;
    ...
    if (!pt.has_partition_table) {
        u_printf("there is no partition table\n");
    } else if (pt.partition_count == 0) {
        u_printf("the partition table is empty\n");
    }
    ...
    u_printf("un-partitioned_space: S(%s%s) NSBOOT(%s%s) NS(%s%s) uf2 { %s }\n",
    ...
    while (read_next_partition(&pt, &p)) {
        u_printf("%3d:", pt.current_partition - 1);

        u_printf("    %08x->%08x S(%s%s) NSBOOT(%s%s) NS(%s%s)",
        ...
        if (p.has_id) {
            u_printf(", id=%016llx", p.partition_id);
        }
        if (p.has_name) {
            u_printf(", \"%s\"", p.name);
        }
